...
function checkcollision(elm1,elm2){
    ...
    return(elm1Rect.right >= elm2Rect.left && elm1Rect.left <=
        elm2Rect.right) && (elm1Rect.bottom >= elm2Rect.top &&
        elm1Rect.top <= elm2Rect.bottom);
}

setInterval(() => {
    if(checkcollision(document.getElementById("bird"),
    document.getElementById("pipe1"))){
        elem.style.top = 520 + "px";
        setTimeout(() => {
            alert("Your Lost !! Your Score is :" +score);
            return;
        }, 10);
        window.location.reload();
    }
    else if(checkcollision(document.getElementById("bird"),
